...
-- We'll migrate all non-zero amounts for the current year (2025) - one month at a time

-- January
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 1, jan_amount
FROM budget_items 
WHERE jan_amount != 0;

-- February
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 2, feb_amount
FROM budget_items 
WHERE feb_amount != 0;

-- March
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 3, mar_amount
FROM budget_items 
WHERE mar_amount != 0;

-- April
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 4, apr_amount
FROM budget_items 
WHERE apr_amount != 0;

-- May
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 5, may_amount
FROM budget_items 
WHERE may_amount != 0;

-- June
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 6, jun_amount
FROM budget_items 
WHERE jun_amount != 0;

-- July
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 7, jul_amount
FROM budget_items 
WHERE jul_amount != 0;

-- August
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 8, aug_amount
FROM budget_items 
WHERE aug_amount != 0;

-- September
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 9, sep_amount
FROM budget_items 
WHERE sep_amount != 0;

-- October
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 10, oct_amount
FROM budget_items 
WHERE oct_amount != 0;

-- November
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 11, nov_amount
FROM budget_items 
WHERE nov_amount != 0;

-- December
INSERT INTO budget_item_periods (budget_item_id, period_year, period_month, amount)
SELECT id, 2025, 12, dec_amount
FROM budget_items 
WHERE dec_amount != 0;
